...
def loop_once(iteration: int) -> None:
    ...
    for rig in rigs:
        ...
        label = []
        if idle_polecats:
            message.append("Please check on stale polecats and 'gt done' them. Note - if a polecat has submitted work to MQ, they need to be 'gt done', regardless of merge conflicts. Any merge conflicts will handled in a new bead and slung to another polecat. Also, if a polecat reports no work on hook, they also need to be 'gt done'. If any polecats are done working but still showing up due to a stale state sync issue, please resolve that too.")
